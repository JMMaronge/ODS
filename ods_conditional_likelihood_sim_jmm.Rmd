...
beta.diff.cov.prob<-vector(length = reps)
beta.x_ibar.est<-vector(length = reps)
beta.x_ibar.cov.prob<-vector(length = reps)
...
beta.diff.cov.prob[i]<-(intervals(fit)$fixed[2,1]<=beta&intervals(fit)$fixed[2,3]>=beta)
beta.x_ibar.est[i]<-fixed.effects(fit)[3]
beta.x_ibar.cov.prob[i]<-(intervals(fit)$fixed[3,1]<=beta&intervals(fit)$fixed[3,3]>=beta)
}
```
...
beta.x_ibar.cov.prob<-vector(length = reps)
beta.int.est<-vector(length = reps)
beta.int.cov.prob<-vector(length = reps)
for(i in 1:reps){
  u<-rnorm(pop.m,mean = 0, sd=sqrt(tau_sq)) #cluster samples
  u1<-rep(u,each=pop.n) # repeat each cluster sample n times
  x<-rnorm(u1,1)
  ...
  beta.int.est[i]<-coef(summary(fit.logit))[1,1]
  beta.int.cov.prob[i]<-((coef(summary(fit.logit))[1,1]-qt(.975, df=2000)*coef(summary(fit.logit))[1,2])<=beta0&(coef(summary(fit.logit))[1,1]+qt(.975, df=2000)*coef(summary(fit.logit))[1,2])>=beta0)
  beta.diff.est[i]<-coef(summary(fit.logit))[2,1]
  beta.diff.cov.prob[i]<-((coef(summary(fit.logit))[2,1]-qt(.975, df=2000)*coef(summary(fit.logit))[2,2])<=beta1&(coef(summary(fit.logit))[2,1]+qt(.975, df=2000)*coef(summary(fit.logit))[2,2])>=beta1)
  beta.x_ibar.est[i]<-coef(summary(fit.logit))[3,1]
  beta.x_ibar.cov.prob[i]<-((coef(summary(fit.logit))[3,1]-qt(.975, df=2000)*coef(summary(fit.logit))[3,2])<=beta1&(coef(summary(fit.logit))[3,1]+qt(.975, df=2000)*coef(summary(fit.logit))[3,2])>=beta1)
...
for(i in 1:reps){
  ...
  beta.diff.est[i]<-coef(fit.clogit)[1]
  ci<-confint(fit.clogit)
  beta.diff.cov.prob[i]<-(ci[1,1]<=beta1&ci[1,2]>=beta1)
}
```
